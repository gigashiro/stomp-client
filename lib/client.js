...
StompClient.prototype.onConnect = function() {
  ...
  frameEmitter.on('MESSAGE', function(frame) {
    var queue = frame.headers.destination.replace(new RegExp('^\/queue\/'), '')
    var subscribed = self.subscriptions[queue];
...
function sendAckNack(acknack, messageId, destination, subscription, transaction) {
  var headers = {
...
StompClient.prototype.ack = function(messageId, destination, subscription, transaction) {
  sendAckNack.call(this, 'ACK', messageId, destination, subscription, transaction);
  return this;
};

StompClient.prototype.nack = function(messageId, destination, subscription, transaction) {
  sendAckNack.call(this, 'NACK', messageId, destination, subscription, transaction);
  return this;
};
